...
| 语音转文字（用于 prompting） | [SuperWhisper](https://superwhisper.com/)（**默认推荐**）、[Typeless](https://typeless.app/)、[Spokenly](https://spokenly.app/) 或[微信输入法](https://z.weixin.qq.com/) | **强烈推荐**，尤其用于 Claude Code / Codex prompting。SuperWhisper 默认推荐；Typeless 输出更整洁但改动较多。微信输入法语音输入免费且无限量（中英文都支持，英文效果其实非常好）。至少试用其中一个（~5 分钟配置好）看看能否提升效率。 |
